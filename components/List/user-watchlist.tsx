...
export default function UserWatchlist({ tab }: UserWatchlistProps) {
  const { movies, tv } = useWatchlistStore((state) => state);
  ...
  const content = list.length ? (
    ...
    <Empty
      title={`No watchlist ${tab === "movie" ? "movie" : "tv show"} yet!`}
    />
  );
  return (
